...
############ Lag variables #####################
data = create_lagged_variables(data, ["temperature"], [12])
...
#fit_vars = ["year_normalized", "month_sin", "month_cos", "temperature_L12"]

fit_vars = ["temperature_L12"]
#################################################
...
#kernel_mean = (
#    ConstantKernel(1.0, (1e-2, 1e2))
#    * ExpSineSquared(length_scale = 12, periodicity = 12, length_scale_bounds = (1e-2, 1e2), periodicity_bounds=(11, 13))
#    + RBF(length_scale = 50, length_scale_bounds = (1e-2, 1e3)) 
...
kernel_season = ConstantKernel(1.0, (1e-2, 1e2)) * ExpSineSquared(
    ...
    periodicity=12.0, 
    length_scale_bounds=(1e-2, 1e2),
    periodicity_bounds=(11, 13) ## add it back to expSine
)

kernel_local = RBF(length_scale=96, length_scale_bounds=(1e-2, 1e3)) # change back to 50

#### Original ####
kernel_noise = WhiteKernel(noise_level=1e-3)
#kernel_noise = WhiteKernel(noise_level=1e-3, noise_level_bounds=(1e-6, 1e1))
...
for val_year in val_years:
    ...
    # Fit GP for variance
    gp_var = GaussianProcessRegressor(kernel=kernel_var, alpha=1e-2, normalize_y=False, n_restarts_optimizer=2)
...
columns_to_keep = ["county", "year", "month", "temperature", "mu", "sigma", "temperature_L12", "quantile_score"]
...
for test_year in [2018, 2019, 2020, 2021, 2022, 2023, 2024]:
    ...
    predict_df[test_year] = pd.DataFrame({
        ...
        "temperature_L12": test_data["temperature_L12"],
        "mu": mu,
        "sigma": sigma
    })
...
combined_df.to_csv(f"/scratch/users/jelazaro/temp_V9_results/baseline_gp/quantile_scores/{county_name}_QS_OG_lags.csv")
