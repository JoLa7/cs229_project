...
fit_vars = ["year", "month",'temp_anom_L1', 'temp_anom_L10', 'soil_temperature_layer_1_L12']
        ...
        for gamma in [0, 0.05, 0.1, 0.15, 0.2]:
            for min_child_weight in [1, 3, 5]:
                ...
                for val_year in [2015, 2016, 2017]:
                    ...
                    y_train[val_year] = np.array(train_data["temp_anom"])

                    X_val[val_year] = np.array(val_data[fit_vars])
                    y_val[val_year] = np.array(val_data["temp_anom"])
...
for test_year in [2018, 2019, 2020, 2021, 2022, 2023, 2024]: # 6 years Mauricio wanted 
    ...
    y_train[test_year] = np.array(train["temp_anom"])
...
for test_year in test_years:
    ...
    for simulation in range(N_simulations):
        ...
        for month in months:
            ...
            for lag in range(1, max_lag + 1):
                ...
                if target_month <= 12:
                    lag_column_name = f"temp_anom_L{lag}"
                    simulated_data.loc[simulated_data["month"] == target_month, lag_column_name] = simulated_temp_anom
    ...
    columns_to_keep = ["county", "year", "month", "temperature", "temp_anom", "simulated_temp_anom", "mu", "sigma", "simulation_number", "hist_temp_avg"]
...
all_simulation_results_df['mu_shifted'] = all_simulation_results_df['hist_temp_avg'] + all_simulation_results_df['mu']
all_simulation_results_df['simulated_temperature'] = all_simulation_results_df['simulated_temp_anom'] + all_simulation_results_df['hist_temp_avg']
...
for _, row in year_month_combinations.iterrows():
    ...
    if len(sim_temp_np) > 0:
        ...
        if test_year <= 2024: # change once we get 2024 data (when 2025 gets introduced this will make more sense)
...
quantile_scores_df.to_csv(f"/scratch/users/jelazaro/temp_V9_results/anom_one_ten_xgbd/quantile_scores/{county_name}_QS_anom_oneten_lags.csv")
all_simulation_results_df.to_csv(f"/scratch/users/jelazaro/temp_V9_results/anom_one_ten_xgbd/simulation_values/{county_name}_simulation_values_anom_oneten_lags.csv")
all_feature_importance_df.to_csv(f"/scratch/users/jelazaro/temp_V9_results/anom_one_ten_xgbd/ft_importance/{county_name}_ft_importance_anom_oneten_lags")
