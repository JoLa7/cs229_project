...
import os
import json
model_params_dir = "/scratch/users/jelazaro/temp_V9_results/model_params/sparse_xgbd"
model_type = "sparse_xgbd"
...
prev_yr_lags = [12]
lagged_vars = ["soil_temperature_layer_1", "leaf_area_index_hi_veg"]

data = create_lagged_variables(data, lagged_vars, prev_yr_lags)
data = data.sort_values(by = ["year", "county", "month"])

### Create list of variables to fit the data ###
fit_vars = ['temperature_L6', 'temperature_L12', 'temperature_L24', 'soil_temperature_layer_1_L12', 'leaf_area_index_hi_veg']
...
quantile_scores_df.to_csv(f"/scratch/users/jelazaro/temp_V9_results/sparse_xgbd/quantile_scores/{county_name}_QS_sparse_lags.csv")
all_simulation_results_df.to_csv(f"/scratch/users/jelazaro/temp_V9_results/sparse_xgbd/simulation_values/{county_name}_simulation_values_sparse_lags.csv")
all_feature_importance_df.to_csv(f"/scratch/users/jelazaro/temp_V9_results/sparse_xgbd/ft_importance/{county_name}_ft_importance_sparse_lags")
