...
def objective(trial):
    ...
    colsample_bytree = trial.suggest_float("colsample_bytree", 0.5, 1.0)  # Fraction of fatures used per tree
...
study_name = "Temp_Forecasting_Opt"  # Unique identifier of the study.
county_filename = args.county.replace(" ", "_")
storage_path = os.path.join(args.output_dir, f'{county_filename}_optuna_study')
storage = "sqlite:///{}.db".format(storage_path)
...
# Log sampler selected by optuna
sampler_log_filepath = os.path.join(args.output_dir, f'{county_filename}_sampler.log')
with open(sampler_log_filepath, 'w') as f:
    print(f"Sampler Used: {type(study.sampler._sampler)}", file = f)
